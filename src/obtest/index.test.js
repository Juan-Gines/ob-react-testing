...
describe('Assertions básicos con boleanos', () => {
...
describe('Assertions básicos en los strings', () => {
	test('El campo debe tener un email', () => {
		...
		expect(email).toMatch('juan@juan.com');
	});
});
...
describe('Assertions básicos en objetos', () => {
	test('Dos objetos son iguales', () => {
		...
		//expect(objA).toBe(objB); no funciona por que los objetos no son iguales hay k mirar dentro todo con toEqual
	...
	test('La propiedad "ancho" del rectángulo debe ser 10', () => {
		const rectangulo = devuelveObjeto();
		expect(rectangulo).toHaveProperty('ancho', 10);
	});
	test('El objeto rectA debe ser un Clase Rectángulo', () => {
		//const rectA = { ancho: 10, alto: 11 }; Esto falla por que no es instancia de Rectángulo
...
describe('Assertions básicos en arrays', () => {
	...
	test('Array contiene id: 4', () => {
		const ar = devuelveArrayObj();
		expect(ar).toContainEqual({ id: 4 }); //toContain no compora objetos
	});
});

describe('Assersions con funciones mock', () => {
